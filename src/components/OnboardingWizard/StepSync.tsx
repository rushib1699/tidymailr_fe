...
export default function StepSync() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setSyncStatus({
        type: 'success',
        message: `Successfully synced ${response.emailCount || 0} emails (max 200 per account)`,
        details: response
      });
    } catch (error) {
      setSyncStatus({
        ...
        <div className="space-y-3 text-sm">
          <div>
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {Object.entries(state.onboardingData.filters || {})
                .filter(([_, enabled]) => enabled)
                .map(([filter, _]) => (
                  <span key={filter} className="px-2 py-1 bg-blue-100 text-blue-800 rounded-md capitalize">
                    {filter}
                  </span>
                ))}
            </div>
          ...
          <div>
            ...
            <span className="ml-2 text-blue-700">
              {state.onboardingData.workingHours?.start || '9:00'} - {state.onboardingData.workingHours?.end || '17:00'}
            </span>
          </div>
          <div>
